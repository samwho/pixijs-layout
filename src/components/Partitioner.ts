...
export default abstract class Partitioner extends Container implements Positioner {
    ...
    constructor(children: DisplayObject[] = [], { debug, center }: { debug?: boolean, center?: boolean } = {}) {
        super()
        this.debug = debug ?? this.debug;
        this.center = center ?? this.center;
        this._group = children;
        this.sortableChildren = true;
        this.zIndex = children.map(child => child.zIndex).reduce((a, b) => Math.min(a, b), Infinity);
    }
    ...
    override removeChildren(beginIndex?: number | undefined, endIndex?: number | undefined): DisplayObject[] {
        let children = this._group.splice(beginIndex ?? 0, endIndex ?? this._group.length);
        this.refresh();
        return children;
    }
